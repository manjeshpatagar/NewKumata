...
function Router() {
  ...
  // -------------------------------
  // REDIRECT HANDLING (AFTER LOAD)
  // -------------------------------
 useEffect(() => {
  if (isAuthLoading) return;

  const publicRoutes = ['/login', '/register', '/forgot-password', '/admin-login', '/'];
  ...
  // User protection
  if (!isAuthenticated && !isGuest && !publicRoutes.includes(pathname)) {
    router.push('/login');
  }

}, [isAuthenticated, isGuest, isAdminLoggedIn, isAuthLoading, pathname, router]);
  ...
  const renderPage = () => {
    ...
    if (pathname === '/admin/categories') return <AdminCategoriesPage onBack={() => router.push('/admin')} />;
    if (pathname === '/admin/notifications') return <AdminNotificationsPage onBack={() => router.push('/admin')} />;
    if (pathname === '/admin/analytics') return <AdminAnalyticsPage onBack={() => router.push('/admin')} />;
  };
