...
interface SmartSearchBarWithImagesProps {
  placeholder?: string;
  className?: string;
}

export function SmartSearchBarWithImages({ placeholder, className = '' }: SmartSearchBarWithImagesProps) {
    ...
    categories.forEach(cat => {
      ...
      subcategories.forEach((subcat, index) => {
        results.push({
          ...
          image: subcat.image,
          icon: subcat.icon,
          trending: index < 2 && (cat.id === 'shops' || cat.id === 'services'), // Mark first 2 as trending
        });
      });
    });
  ...
  const handleSearch = (result: SearchResult) => {
    ...
    router.push(`/subcategory?categoryId=${result.categoryId}&categoryName=${encodeURIComponent(result.name)}`);
  };
  ...
  const trending = allResults.filter(r => r.trending).slice(0, 6);
                    ...
                    {trending.map((result) => {
                      ...
                      return (
                        <div
                          ...
                          className="relative rounded-lg overflow-hidden cursor-pointer group shadow-md hover:shadow-xl transition-all duration-200"
                        >
                          {isSubcategory && result.image ? (
                            <>
                              ...
                              {/* Strong gradient for better text visibility */}
                              <div className="absolute inset-0 bg-gradient-to-t from-black via-black/60 to-black/20"></div>
                              
                              {/* Icon - Top Left */}
                              <div className="absolute top-2 left-2 text-xl drop-shadow-[0_2px_8px_rgba(0,0,0,0.9)]">
                                {result.icon}
                              </div>
                              ...
                              <div className="absolute bottom-0 left-0 right-0 p-2">
                                <div className="bg-black/50 backdrop-blur-sm rounded px-2 py-1">
                                  <p className="text-white text-xs truncate" style={{ textShadow: '0 2px 4px rgba(0,0,0,0.9)' }}>
                                    {result.name}
                                  </p>
                                  <p className="text-white/90 text-xs truncate" style={{ textShadow: '0 1px 3px rgba(0,0,0,0.9)' }}>
                                    {result.category}
                                  </p>
                                </div>
                              </div>
                              
                              <Badge className="absolute top-2 right-2 text-xs bg-orange-500 text-white border-0 shadow-lg">
                                🔥
                              </Badge>
                            </>
                          ) : (
                            <div className="aspect-video bg-gradient-to-br from-blue-500 to-purple-600 flex flex-col items-center justify-center p-3">
                              <p className="text-white text-xs text-center truncate w-full" style={{ textShadow: '0 2px 4px rgba(0,0,0,0.5)' }}>{result.name}</p>
                              <p className="text-white/90 text-xs" style={{ textShadow: '0 1px 3px rgba(0,0,0,0.5)' }}>{result.category}</p>
                              <Badge className="absolute top-2 right-2 text-xs bg-orange-500 text-white border-0">
                                🔥
                              </Badge>
                            </div>
                          )}
              ...
              {Object.entries(groupedResults).map(([category, results]) => (
                <div key={category} className="mb-4">
                  ...
                  <div className="space-y-1">
                    {results.slice(0, query ? results.length : 5).map((result) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {isSubcategory ? (
                            result.image ? (
                              <div className="relative w-14 h-14 rounded-lg overflow-hidden flex-shrink-0 group-hover:scale-105 transition-transform shadow-md">
                                <ImageWithFallback
                                  src={result.image}
                                  alt={result.name}
                                  className="w-full h-full object-cover"
                                />
                                {/* Stronger gradient for icon visibility */}
                                <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/30 to-transparent"></div>
                                {/* Icon with strong shadow */}
                                <div className="absolute bottom-0.5 right-0.5 text-xl leading-none drop-shadow-[0_2px_6px_rgba(0,0,0,0.9)]">
                                  {result.icon}
                                </div>
                              </div>
                            ) : (
                              <div className={`w-14 h-14 rounded-lg ${getCategoryColor(result.categoryId)} group-hover:scale-105 transition-transform flex items-center justify-center text-2xl shadow-md`}>
                                {result.icon || '📍'}
                              </div>
                            )
                          ) : (
                            <div className={`p-3 rounded-lg ${getCategoryColor(result.categoryId)} group-hover:scale-110 transition-transform`}>
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
                              {result.subcategory && !isSubcategory && (
                                <span className="truncate">{result.subcategory}</span>
                              )}
                              {result.location && (
                                <>
                                  {result.subcategory && !isSubcategory && <span>•</span>}
                    ...
                    {!query && results.length > 5 && (
                      ...
                      >
                        {t('showMore') || 'Show More'} ({results.length - 5} {t('more') || 'more'})
                      </button>
                    )}
                  </div>
                </div>
