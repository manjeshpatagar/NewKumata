...
export function AdminCategoriesPage({
  ...
  const currentCategories =
    selectedTab === "shops" ? shopCategories : adCategories;
  ...
  const stats = {
    shops: {
      total: shopCategories.length,
      active: shopCategories.filter((c) => c.isActive).length,
      items: shopCategories.length, // or some real count later
    },
    ads: {
      total: adCategories.length,
      active: adCategories.filter((c) => c.isActive).length,
      items: adCategories.length,
    },
  };
  ...
  const handleAddCategory = async () => {
    try {
      ...
      form.append(
        "type",
        selectedTab === "shops" ? "business" : "advertisement"
      );
      form.append("image", newCategoryFile);

      const res = await categoryApi.create(form);
      // assuming res = { success, message, data: category }
      const created: Category = res.data;

      if (selectedTab === "shops") {
        setShopCategories((prev) => [...prev, created]);
      } else {
        setAdCategories((prev) => [...prev, created]);
      }
  ...
  const handleEditCategory = async () => {
    ...
    try {
      ...
      // Optionally allow toggling active state via separate UI later
      form.append("isActive", String(editingCategory.isActive));
      if (editImageFile) {
  ...
  const toggleCategoryStatus = async (id: string) => {
    try {
      // find in either list
      const all = [...shopCategories, ...adCategories];
      const found = all.find((c) => c._id === id);
      if (!found) return;

      const form = new FormData();
      form.append("isActive", String(!found.isActive));
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-center gap-3 flex-1">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  {/* Backend doesn’t have itemCount, so 0 for now */}0 items
                </p>
              </div>
            </div>
    ...
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-gray-50 via-indigo-50/30 to-purple-50/30 dark:from-gray-950 dark:via-indigo-950/30 dark:to-purple-950/30">
      ...
      <div className="sticky top-0 z-20 backdrop-blur-xl bg-white/90 dark:bg-gray-900/90 shadow-lg border-b border-gray-200/50 dark:border-gray-800/50">
        <div className="max-w-7xl mx-auto px-4 py-4 md:py-5">
          <div className="flex items-center justify-between flex-wrap gap-3">
            ...
            <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
              <DialogTrigger asChild>
                <Button className="bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white border-0 shadow-lg hover:shadow-xl transition-all hover:scale-105 rounded-xl">
                  <Plus className="w-4 h-4 mr-2" />
                  Add Category
                </Button>
              </DialogTrigger>
      ...
      <ScrollArea className="flex-1">
        <div className="max-w-7xl mx-auto px-4 py-6 pb-24">
          <Tabs
            value={selectedTab}
            onValueChange={(v) => setSelectedTab(v as "shops" | "ads")}
            className="w-full"
          >
            <TabsList className="grid w-full grid-cols-2 mb-6 bg-white dark:bg-gray-900 shadow-md rounded-xl p-1">
              <TabsTrigger
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              category and may affect existing listings.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
