...
const normalize = (id: any) => String(id || "");
...
export function FavoritesProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    try {
      ...
      if (stored) {
        ...
        // normalize all ids
        const fixed = parsed.map((f: FavoriteItem) => ({
          ...f,
          refId: normalize(f.refId),
          favouriteId: normalize(f.favouriteId),
        }));
  ...
  ------------------------------------------*/
  const addFavorite = (item: FavoriteItem) => {
    const refId = normalize(item.refId);
    const favouriteId = normalize(item.favouriteId);

    setFavorites((prev) => {
      if (prev.some((f) => f.refId === refId)) return prev;

      return [...prev, { ...item, refId, favouriteId }];
    });
  };
  ...
  ------------------------------------------*/
  const removeFavorite = (favouriteId: string) => {
    const id = normalize(favouriteId);

    setFavorites((prev) => prev.filter((f) => f.favouriteId !== id));
  };
  ...
  ------------------------------------------*/
  const isFavorite = (refId: string) => {
    const id = normalize(refId);
    return favorites.some((f) => f.refId === id);
  };
  ...
  const getFavouriteId = (refId: string) => {
    const id = normalize(refId);
    const found = favorites.find((f) => f.refId === id);
    return found ? found.favouriteId : null;
  };
  ...
  const toggleFavorite = (item: FavoriteItem) => {
    ...
    if (isFavorite(refId)) {
      const favId = getFavouriteId(refId);
      if (favId) removeFavorite(favId);
    } else {
      addFavorite(item);
    }
